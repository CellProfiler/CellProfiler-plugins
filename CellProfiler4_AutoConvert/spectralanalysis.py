import numpy as np
import scipy.ndimage, scipy.misc, scipy.signal, scipy.interpolate, scipy.linalg, scipy.stats
...
class SpectralAnalysis(cellprofiler_core.module.Module):
    ...
    def create_settings(self):
        self.input_image_name = ImageSubscriber("Input image", "None")
        self.objects_name = LabelSubscriber("Objects name", "Nuclei")
    ...
    def run(self, workspace):    
        ...
        for obj_label in obj_indices:
            ...
            for key, value in result['orientation_mixed'].items():
            ...
            for key, value in result['distance_mixed'].items():
            ...
            for key, value in result['orientation_detailed'].items():
                if key in orientationFeatures:
                    if key in OD_VALUES:
    ...
    def get_measurement_columns(self, pipeline):
        ...
        return [(self.objects_name.value, 'SpectralAnalysis_OrientationMixed_' + key, COLTYPE_FLOAT) for key in orientationKeys] + \
               [(self.objects_name.value, 'SpectralAnalysis_DistanceMixed_' + key, COLTYPE_FLOAT) for key in distanceKeys] + \
               [(self.objects_name.value, 'SpectralAnalysis_OrientationDetailed_' + key, COLTYPE_FLOAT) for key in orientationKeys]
