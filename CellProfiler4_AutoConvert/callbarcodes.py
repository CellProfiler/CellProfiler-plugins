...
""".format(**{
        ...
        #
        # The LabelSubscriber is similar to the ImageSubscriber.
...
Enter the name to be given to the barcode score image.""")
    ...
    def run(self, workspace):
        ...
        workspace.measurements.add_measurement(self.input_object_name.value, '_'.join([C_CALL_BARCODES,'MatchedTo_Score']),
                                     scorelist)
        if self.wants_call_image:
            workspace.image_set.add(self.outimage_calls_name.value,cellprofiler_core.image.Image(pixel_data_call.astype("uint16"),
                                                                                      convert = False ))
        if self.wants_score_image:
            workspace.image_set.add(self.outimage_score_name.value,cellprofiler_core.image.Image(pixel_data_score.astype("uint16"),
                                                                                        convert = False ))
        #
    ...
    def get_measurement_columns(self, pipeline):
        ...
        return [(input_object_name, '_'.join([C_CALL_BARCODES,'BarcodeCalled']), cellprofiler_core.measurement.COLTYPE_VARCHAR),
                (input_object_name, '_'.join([C_CALL_BARCODES,'MatchedTo_Barcode']), cellprofiler_core.measurement.COLTYPE_VARCHAR),
                (input_object_name, '_'.join([C_CALL_BARCODES,'MatchedTo_ID']), cellprofiler_core.measurement.COLTYPE_INTEGER),
                (input_object_name, '_'.join([C_CALL_BARCODES,'MatchedTo_GeneCode']), cellprofiler_core.measurement.COLTYPE_VARCHAR),
                (input_object_name, '_'.join([C_CALL_BARCODES,'MatchedTo_Score']), cellprofiler_core.measurement.COLTYPE_FLOAT)]
