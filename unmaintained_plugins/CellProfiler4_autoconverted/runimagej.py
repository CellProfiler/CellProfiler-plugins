...
class RunImageJ(cellprofiler_core.module.Module):
    category = "Advanced"
    ...
    def run(self, workspace):
        ...
        for details, setting in zip(self.output_details, self.output_settings.settings):
            ...
            if isinstance(setting, cellprofiler_core.setting.text.ImageName):
                image = cellprofiler_core.image.Image(value)
    ...
    def _connect(self, ij_module=None):
        ...
        self.ij_module = cellprofiler_core.setting.choice.Choice(
            "ImageJ module",
    ...
    def _create_ij_settings(self, module_name):
        ...
        for output in details["outputs"]:
            ...
            if raw_type in IMAGE_TYPES:
                ...
                setting = cellprofiler_core.setting.text.ImageName(text)
                self.output_details.append(output)
                self.output_settings.append("output_" + output["name"], setting)

        self.input_count = cellprofiler_core.setting.HiddenCount(self.input_details, "input count")
    ...
    def _make_setting(self, input_):
        ...
        if raw_type in BOOL_TYPES:
            return cellprofiler_core.setting.Binary(text, value)

        if raw_type in COLOR_TYPES:
            return cellprofiler_core.setting.Color(text, value)

        if raw_type in FILE_TYPES:
            if style.startswith("directory"):
                # TODO: Massage non-None value to CellProfiler-friendly string.
                return cellprofiler_core.setting.text.Directory(text)

            # "open" or "save" or unspecified
            # TODO: Use a fancier combination of widgets.
            return cellprofiler_core.setting.Pathname(text, value if value else '')

        if raw_type in FLOAT_TYPES:
            return cellprofiler_core.setting.text.Float(text, self._clamp(value, minval), minval, maxval)

        if raw_type in IMAGE_TYPES:
            return cellprofiler_core.setting.subscriber.ImageSubscriber(text)

        if raw_type in INTEGER_TYPES:
            return cellprofiler_core.setting.text.Integer(text, self._clamp(value, minval), minval, maxval)

        if raw_type in TEXT_TYPES:
            choices = input_["choices"]
            if choices is None:
                if style.startswith("text area"):
                    return cellprofiler_core.setting.text.Text(text, value, multiline=True)
                return cellprofiler_core.setting.text.Text(text, value)

            return cellprofiler_core.setting.choice.Choice(text, choices)
    ...
    def _output_value(self, setting, id, ij):
        if isinstance(setting, cellprofiler_core.setting.text.ImageName):
            ...
            pil = Image.open(io.StringIO(data))
            return np.array(pil)
