...
__doc__ = """\
UnetPredict calculates pixel wise class predictions using an UNet 
network. The default network model is trained to identify nuclei, background 
and the nuclei boundary. Predictions are returned as three channel images: 

* red channel stores background predictions
* green channel stores nuclei predictions
* blue channel stores boundary predictions 

In the simplest use case, the predictions are converted to gray value images 
using the module ColorToGray. The module IdentifyPrimaryObjects can be 
used to identify example images in the nuclei predictions (green channel). 
...
class UnetPredict(cellprofiler.module.ImageProcessing):
    module_name = "UnetPredict"
    ...
    def run(self, workspace):
        ...
        self.function = lambda input_image: unet_prediction(model, input_image)

        super(UnetPredict, self).run(workspace)
...
def unet_initialize(input_shape):
    ...
    if not os.path.exists(weights_filename):
        ...
        id = "1I9j4oABbcV8EnvO_ufACXP9e4KyfHMtE"
...
def unet_prediction(model, input_image):
